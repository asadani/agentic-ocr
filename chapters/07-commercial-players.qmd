...
## The platform tier — Databricks, Snowflake, Microsoft Fabric
...
Snowflake matches Databricks' move with **`AI_PARSE_DOCUMENT`**, a Cortex AI function that extracts text, data, layout elements, and images from documents stored on internal or external stages [@snowflakeparsedocument]. The naming convention is not an accident — the two products converged on the same SQL-function shape, the same chainable-with-other-AI-functions architecture, and the same "agentic OCR as a primitive inside the data platform" positioning.
